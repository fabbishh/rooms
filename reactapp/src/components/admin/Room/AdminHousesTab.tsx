...
const AdminHousesTab = (props: Props) => {
    ...
    const columns = [
        ...
        {
            cell: (row: any) => <AdminPromoModal id={row.roomGroupId} type="room"/>,
            ignoreRowClick: true,
            allowOverflow: true,
            //button: true,
            center: true
    ...
    const fetchData = async () => {
        try {
            var response = await api.post("room/GetRoomTableData", { pageNumber: 1, pageSize: 10, housingType: 1 });
    ...
    return (
        <>
            <div className='flex gap-2'>
                <SaveRoomModal title="Создание отдельного жилья" buttonName='Создать' onSave={handleSaveRoom} isAdmin={true} type="house"/>
